...
This document is a draft of the current version ("v1") of the rpc4s wire
protocol over HTTP. This draft is EXPERIMENTAL and subject to change.
v1 of rpc4s is not released or considered stable; instead, this
document lists planned changes which may be available in prerelease
distributions.

## Overview

The rpc4s wire protocol is a simple RPC protocol based on HTTP and
...
To use rpc4s, developers first define their APIs using proto files,
then use rpc4s tools to generate the client and the server libraries.
The generated libraries implement the rpc4s wire protocol, using the
...
The rpc4s wire protocol supports both binary and JSON encodings of
proto messages, and works with any HTTP client and any HTTP version.

The wire protocol and this specification can be considered as a fork
of [twirp wire protocol](https://github.com/twitchtv/twirp/blob/master/docs/spec_v5.md).

### URLs

In [BNF syntax](https://tools.ietf.org/html/rfc5234), rpc4s's URLs
...
The rpc4s wire protocol uses HTTP URLs to specify the RPC
endpoints on the server for sending the requests. Such direct mapping
makes the request routing simple and efficient. The rpc4s URLs have
the following components.

* **Base-URL** is the virtual location of a rpc4s API server, which is typically
...
rpc4s always uses HTTP POST method to send requests, because it
closely matches the semantics of RPC methods.

The **Request-Headers** are normal HTTP headers. The rpc4s wire
...
* _TODO_ Add CT for streaming request
...
rpc4s wire protocol uses the following headers.
...
* _TODO_ Add CT for streaming response
...
_TODO_ Create sub section with streaming examples.
...
rpc4s error responses are always JSON-encoded, regardless of
...
rpc4s errors are a JSON object with the keys:

* **code**: One of the rpc4s error codes as a string.
...
rpc4s errors always include an error code. This code is represented
...
| rpc4s Error Code    | HTTP Status | Description
| ------------------- | ----------- | -----------
...
| bad_route           | 404 | The requested URL path wasn't routable to a rpc4s service and method. This is returned by generated server code and should not be returned by application code (use "not_found" or "unimplemented" instead).
...
| internal            | 500 | When some invariants expected by the underlying system have been broken. In other words, something bad happened in the library or backend service. rpc4s specific issues like wire and serialization problems are also reported as "internal" errors.
